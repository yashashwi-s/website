...
export default function Preloader() {
  ...
  // "fast" = shortened count-up + exit, used for reduced-motion visitors and
  // repeat navigations within the same session so the full ~2.3s choreography
  // only ever plays once per visit, not on every load.
  ...
  useEffect(() => {
    ...
    try {
      ...
      // sessionStorage can throw in some privacy modes — fall back to full intro.
    }
    const fastMode = Boolean(shouldReduceMotion) || seenThisSession;
    setFast(fastMode);

    const duration = fastMode ? 360 : 1200; // ms
    const holdTime = fastMode ? 80 : 200; // ms, pause once progress hits 100
      ...
      if (currentStep >= steps) {
        ...
        setTimeout(() => {
          ...
          try {
            ...
            // Ignore — worst case the full intro just replays next load.
          }
      ...
      {isLoading && (
        <div className="fixed inset-0 z-[9999] pointer-events-none flex items-center justify-center">
          ...
          <motion.div
            ...
            transition={{ duration: fast ? 0.45 : 0.9, ease: [0.76, 0, 0.24, 1] }}
          ...
          >
            ...
            <div className="relative overflow-hidden h-[20vw] px-8 flex items-center justify-center">
              <motion.div
                ...
                transition={{ duration: fast ? 0.35 : 0.8, ease: "easeOut" }}
            ...
            <div className="absolute bottom-10 left-10 md:bottom-20 md:left-20 overflow-hidden">
              <motion.p
                ...
                transition={{ delay: fast ? 0.05 : 0.2 }}
