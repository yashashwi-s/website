...
// A generous ceiling so a script can't grow the hash without bound. Existing subscribers are
// always allowed through (re-submit is a no-op); only brand-new addresses past the cap are
// refused, and Fadeo is nowhere near this.
...
export async function addSubscriber(email, source = "site") {
  ...
  return withRedis(async (r) => {
    // Only gate genuinely new addresses; a re-submit of an existing one always succeeds.
    if (!(await r.hexists(SUBSCRIBERS_KEY, clean)) && (await r.hlen(SUBSCRIBERS_KEY)) >= MAX_SUBSCRIBERS) {
