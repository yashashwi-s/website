...
// How many issued promo keys have ever pinged activation (LicenseManager.pingActivation),
// plus how many were issued at all. Activations are the only server-side evidence that a
// claimed key was pasted into a real running copy of Fadeo, as opposed to just requested
// from the web form -- which is the difference between "N people installed it" and "N
// people filled in their email". Deliberately NOT part of promoState(): that one is served
// publicly on the product page, and this pair is an internal funnel metric.
export async function activationStats() {
  if (!kvConfigured()) return null;
  return withRedis(async (r) => ({
    activated: await r.scard(ACTIVATED_KEY),
    issued: await r.hlen(ISSUED_KEY),
    reclaimed: await r.scard(RECLAIMED_KEY),
  }));
}
